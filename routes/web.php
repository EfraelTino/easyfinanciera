...
require_once './app/Controllers/CreditController.php';
...
$creditController = new CreditController($db);
$router->get('/', function () {
...
$router->get('/occupations', function () use ($userController) {
    echo $userController->allOccupations();
});

/**CREDIT ROUTES */
$router->post('/create-credit', function () use ($creditController) {
    $data = json_decode(file_get_contents('php://input'), true);
    echo $creditController->createCredit(['body' => $data]);
});
